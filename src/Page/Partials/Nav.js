  ...
  return (
    ...
    <section className=" w-full py-4 sm:px-4 lg:px-8 flex flex-wrap items-center  ">
      ...
      {isMobile ? (<>
        ...
        {show && <Sidebar  onClick={() => setShow(!show)} show={show} navItems={navItems} buttons={buttons} drop={drop} />}
        </>
      ) : (
        <section
          ...
          <ul className=" pl-0 mt-3 space-x-4 font-semibold mb-2  flex flex-col  lg:mt-0 lg:mb-0 lg:flex-row">
            {navItems.map((item) => (
              <li
                ...
                onClick={() =>
                  item.label === "Drop" ? null : item.label === 'home' ? navigate('/') : navigate(`/${item.label}`)
                }
              >
                {item.label === "Drop" ? (
                  <Tooltip
