...
const ProductCard = ({ product, onProductRemove }) => {
  // Initialize quantity from local storage or default to 1
  const initialQuantity =
    parseInt(localStorage.getItem(`product_${product.id}_quantity`), 10) || 1;
  ...
  const handleRemoveProduct = () => {
    onProductRemove(product.id); // Pass the product id to the parent component
  };
  ...
  return (
    <section className="flex flex-col md:flex-row items-center shadow py-4 space-y-4 md:h-44 h-48 justify-between px-4">
      ...
      <TrashIcon
        ...
        style={"hidden md:flex hover:outline outline-1 rounded p-1"}
      />
      ...
      <section className="flex justify-between items-center w-full md:hidden">
        ...
        <TrashIcon
          onClick={handleRemoveProduct}
          style={`hover:outline outline-1 rounded p-1`}
        />
      </section>
    </section>
  );
